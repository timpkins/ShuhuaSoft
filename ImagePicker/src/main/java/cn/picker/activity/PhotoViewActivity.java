...
import cn.picker.R;
import cn.picker.adapter.PhotoViewAdapter;
...
public class PhotoViewActivity extends AppCompatActivity implements OnClickListener {
    ...
    public void onClick(View v) {
        if (v == ivSelectCancel) {
            ...
            if (result) {
                ...
                // 添加失败,超出可选照片上限
                ivPhotoSelected.setImageResource(R.mipmap.compose_photo_preview_default);
    ...
    private void changePhotoSelectStatus(int position) {
        if (PhotoMessage.isPhotoSelected(photoList.get(position))) {
            ivPhotoSelected.setImageResource(R.mipmap.compose_photo_preview_right);
        } else {
            ivPhotoSelected.setImageResource(R.mipmap.compose_photo_preview_default);
        }
    }
